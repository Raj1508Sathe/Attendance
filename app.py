...
if not st.session_state.authenticated:
    st.title("🔐 Computer Engineering Login")
...
with st.sidebar:
    ...
    view = st.radio("Navigation", ["📊 Dashboard", "📝 Records", "📍 Map"])
    if st.button("Logout"):
        st.session_state.authenticated = False
        st.rerun()

st.markdown("<h1 style='text-align:center; color:#00ffff;'>Command Center</h1>", unsafe_allow_html=True)

# ----------------------------------------
# 5. LIQUID DASHBOARD ELEMENTS
# ----------------------------------------
if view == "📊 Dashboard":
    # Monitoring Table (Liquid Width)
    ...
    with m2:
        st.markdown("<div class='glass-card'>", unsafe_allow_html=True)
        st.metric("Active Students", len(df_monitor[df_monitor["Status"]=="Present"]))
        st.markdown("</div>", unsafe_allow_html=True)
    with m3:
        st.markdown("<div class='glass-card'>", unsafe_allow_html=True)
        st.metric("IoT Health", "Optimal")
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    fig.update_layout(
        autosize=True, # Key for liquid display
        margin=dict(l=0, r=0, t=0, b=0),
        paper_bgcolor='rgba(0,0,0,0)', 
        plot_bgcolor='rgba(0,0,0,0)'
    )
    st.plotly_chart(fig, use_container_width=True) # Forces chart to fill the container
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    st.markdown("<div class='glass-card'>", unsafe_allow_html=True)
    st.map(pd.DataFrame({"lat": [18.4485], "lon": [73.8275]}), use_container_width=True)
    st.markdown("</div>", unsafe_allow_html=True)

# ----------------------------------------
# 6. FLOATING CHATBOT (Liquid position)
# ----------------------------------------
query = st.chat_input("Ask about JSPM NTC or Attendance...")
if query:
    # (Chatbot logic same as before)
    st.markdown(f"<div class='floating-chatbot'><b>AI:</b> {query} (Sample Response)</div>", unsafe_allow_html=True)
    }
    .floating-chatbot {
        position: fixed;
        bottom: 80px;
...
if query:
    ...
    """, unsafe_allow_html=True)afe_allow_html=True)
